...
public class LignePanier {
    ...
    private double                garantiePrix        = 0.0;
    private String                numeroSerie         = null;  // n° série pour consoles/accessoires
    ...
    public double  getGarantiePrix()    { return garantiePrix; }
    public boolean isExtensionGarantie(){ return typeGarantieId != null; }
    public String  getNumeroSerie()     { return numeroSerie; }

    /** Total ligne produit (hors garantie) */
    public String getPrixAffiche() {
        return String.format("%.2f €", prix.get() * quantite.get());
    }

    /** Total ligne avec garantie incluse (garantie × quantité) */
    public double getTotalLigne() {
        return prix.get() * quantite.get() + garantiePrix * quantite.get();
    }

    // ── Setters ──────────────────────────────────────────────────
    public void setQuantite(int q) { quantite.set(q); }
    public void setGarantie(Long id, String label, double prix) {
        this.typeGarantieId = id;
        this.garantieLabel.set(label != null ? label : "");
        this.garantiePrix   = prix;
    }
    /** Rétro-compat sans prix (garantie gratuite ou prix inconnu) */
    public void setGarantie(Long id, String label) {
        setGarantie(id, label, 0.0);
    }
    public void setNumeroSerie(String ns) { this.numeroSerie = ns; }
