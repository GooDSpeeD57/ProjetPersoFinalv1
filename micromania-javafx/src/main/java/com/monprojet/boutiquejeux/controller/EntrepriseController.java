...
import com.monprojet.boutiquejeux.dto.magasin.HoraireMagasinDto;
import com.monprojet.boutiquejeux.dto.magasin.HoraireMagasinRequest;
...
public class EntrepriseController {
    ...
    @FXML private VBox              vboxHoraires;

    // ── Horaires (grille construite dynamiquement) ───────────────
    private record LigneHoraire(CheckBox ferme, TextField ouv, TextField ferm) {}
    private final LigneHoraire[] lignesHoraires = new LigneHoraire[8]; // index 1–7
    ...
    public void initialize() {
        ...
        tableMagasins.getSelectionModel().selectedItemProperty().addListener(
                (obs, old, mag) -> {
                    if (mag != null) {
                        chargerFormulaireMagasin(mag);
                        if (mag.id != null) chargerHoraires(mag.id);
                    }
                });
        ...
        // Construire la grille des horaires AVANT viderFormulaireMagasin()
        // (viderFormulaireMagasin → reinitialiserHoraires → lignesHoraires[j] → NPE si tableau vide)
        buildHoraireRows();
    ...
    void handleEnregistrerMagasin() {
        ...
        List<HoraireMagasinRequest> horaireReqs = buildHoraireRequests();
        if (horaireReqs == null) return; // validation échouée, message déjà affiché
        ...
        new Thread(() -> {
            try {
                ...
                final Long idMagasin = result.id;
                // Sauvegarder les horaires (PUT bulk)
                api.put("/magasins/" + idMagasin + "/horaires", horaireReqs);
    ...
    // ─────────────────────────────────────────────────────────────
    // HORAIRES — construction de la grille
    // ─────────────────────────────────────────────────────────────

    /** Construit les 7 lignes Lun-Dim et les ajoute dans vboxHoraires. */
    private void buildHoraireRows() {
        GridPane gp = new GridPane();
        gp.setHgap(8);
        gp.setVgap(6);

        ColumnConstraints cJour  = new ColumnConstraints(80, 80, 80);
        ColumnConstraints cFerme = new ColumnConstraints(70, 70, 70);
        ColumnConstraints cOuv   = new ColumnConstraints(65, 65, 65);
        ColumnConstraints cDash  = new ColumnConstraints(14, 14, 14);
        ColumnConstraints cFerm  = new ColumnConstraints(65, 65, 65);
        gp.getColumnConstraints().addAll(cJour, cFerme, cOuv, cDash, cFerm);

        for (int j = 1; j <= 7; j++) {
            Label lbl = new Label(JOURS[j]);
            lbl.getStyleClass().add("field-label");

            CheckBox ferme = new CheckBox("Fermé");
            ferme.getStyleClass().add("checkbox-dark");

            TextField ouv = new TextField("10:00");
            ouv.setPromptText("HH:mm");
            ouv.getStyleClass().add("text-field-dark");
            ouv.setMaxWidth(65);

            Label dash = new Label("→");
            dash.setStyle("-fx-text-fill:#888;");

            TextField ferm = new TextField("19:30");
            ferm.setPromptText("HH:mm");
            ferm.getStyleClass().add("text-field-dark");
            ferm.setMaxWidth(65);

            // Désactiver ouv/ferm si Fermé est coché
            final TextField ouvF = ouv, fermF = ferm;
            ferme.selectedProperty().addListener((obs, o, selected) -> {
                ouvF.setDisable(selected);
                fermF.setDisable(selected);
            });
            ...
            lignesHoraires[j] = new LigneHoraire(ferme, ouv, ferm);
        }
        vboxHoraires.getChildren().setAll(gp);
    }

    /** Charge les horaires de l'API et remplit la grille. */
    private void chargerHoraires(Long idMagasin) {
        new Thread(() -> {
            try {
                List<HoraireMagasinDto> list = api.get(
                        "/magasins/" + idMagasin + "/horaires",
                        new TypeReference<List<HoraireMagasinDto>>() {});
                final List<HoraireMagasinDto> safe = list != null ? list : List.of();
                Platform.runLater(() -> {
                    reinitialiserHoraires();
                    for (HoraireMagasinDto h : safe) {
                        if (h.jourSemaine < 1 || h.jourSemaine > 7) continue;
                        LigneHoraire ligne = lignesHoraires[h.jourSemaine];
                        ligne.ferme().setSelected(h.ferme);
                        ligne.ouv().setDisable(h.ferme);
                        ligne.ferm().setDisable(h.ferme);
                        if (!h.ferme) {
                            ligne.ouv().setText(h.ouvertureHHMM());
                            ligne.ferm().setText(h.fermetureHHMM());
                        }
                    }
                });
            } catch (Exception ignored) {
                // Si le magasin n'a pas encore d'horaires, on laisse les valeurs par défaut
            }
        }).start();
    }

    /** Réinitialise la grille aux valeurs par défaut (10:00 → 19:30, ouvert). */
    private void reinitialiserHoraires() {
        for (int j = 1; j <= 7; j++) {
            LigneHoraire l = lignesHoraires[j];
            if (l == null) continue; // pas encore construit (appel avant buildHoraireRows)
            l.ferme().setSelected(false);
            l.ouv().setDisable(false);
            l.ferm().setDisable(false);
            l.ouv().setText("10:00");
            l.ferm().setText("19:30");
        }
    }

    /**
     * Lit la grille horaires et construit la liste de requêtes.
     * @return null si une valeur HH:mm est invalide (message affiché)
     */
    private List<HoraireMagasinRequest> buildHoraireRequests() {
        List<HoraireMagasinRequest> list = new ArrayList<>();
        for (int j = 1; j <= 7; j++) {
            LigneHoraire l = lignesHoraires[j];
            HoraireMagasinRequest req = new HoraireMagasinRequest();
            req.jourSemaine = j;
            req.ferme = l.ferme().isSelected();
            if (!req.ferme) {
                String ouv  = l.ouv().getText().trim();
                String ferm = l.ferm().getText().trim();
                if (!ouv.matches("\\d{2}:\\d{2}") || !ferm.matches("\\d{2}:\\d{2}")) {
                    AlertHelper.warn("Horaires",
                            "Format invalide pour " + JOURS[j] + " : utilisez HH:mm (ex: 10:00).");
                    return null;
                }
                req.heureOuverture = ouv;
                req.heureFermeture = ferm;
            }
            list.add(req);
        }
        return list;
