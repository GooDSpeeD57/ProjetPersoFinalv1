...
public class DepotController {
    ...
    private void colorierLignesStatut(TableView<CommandeSummaryDto> table) {
        table.setRowFactory(tv -> new TableRow<>() {
            @Override protected void updateItem(CommandeSummaryDto item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) { setStyle(""); return; }
                String s = item.statut != null ? item.statut : "";
                String style;
                if      ("PAYEE".equals(s))                             style = "-fx-background-color:#1a2a1a;";
                else if ("PREPARATION".equals(s))                       style = "-fx-background-color:#2a2a0a;";
                else if ("EXPEDIEE".equals(s))                          style = "-fx-background-color:#0a1a2a;";
                else if ("LIVREE".equals(s) || "RETIREE".equals(s))    style = "-fx-background-color:#0a1a0a;";
                else if ("ANNULEE".equals(s))                           style = "-fx-background-color:#2a0a0a;";
                else                                                    style = "";
                setStyle(style);
            }
        });
