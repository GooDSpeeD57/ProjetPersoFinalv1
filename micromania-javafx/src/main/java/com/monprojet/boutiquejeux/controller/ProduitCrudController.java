...
public class ProduitCrudController {
    ...
    @FXML private TableColumn<ProduitVariantDto, String> colVEan;
    ...
    @FXML private TextField                  editVSkuField;
    @FXML private TextField                  editVEanField;
    ...
    /** ID du variant à re-sélectionner après un refresh (suite à une sauvegarde). */
    private Long   variantIdToReselect   = null;
    ...
    public void initialize() {
        ...
        tableProduits.getSelectionModel().selectedItemProperty().addListener((obs, o, n) -> {
            if (n != null && mode != Mode.NOUVEAU) {
                // Définir immédiatement produitIdSelec (synchrone) pour éviter
                // toute race condition si l'utilisateur clique avant la fin de chargerDetail
                produitIdSelec = n.id;
                mode = Mode.EDIT;
                chargerDetail(n.id);
            }
        ...
        colVEan.setCellValueFactory(c ->
                new SimpleStringProperty(c.getValue().ean != null ? c.getValue().ean : ""));
    ...
    private void handleSauvegarderVariant() {
        ...
        String skuSaisi = emptyToNull(editVSkuField.getText());
        dto.sku             = skuSaisi != null ? skuSaisi : sel.sku;   // éditable, fallback sur l'original
        dto.ean             = emptyToNull(editVEanField.getText());
        ...
        variantIdToReselect = variantId;   // mémoriser pour re-sélectionner après refresh
        Task<Void> task = new Task<>() {
            @Override protected Void call() throws Exception {
    ...
    private void peuplerFormEditVariant(ProduitVariantDto v) {
        ...
        editVSkuField.setText(v.sku != null ? v.sku : "");
        editVEanField.setText(v.ean != null ? v.ean : "");
    ...
    private void handleAjouterVariant() {
        ...
        if (mode == Mode.EDIT && produitIdSelec == null) {
            // Cas rare : sélection en cours de chargement — bloquer plutôt que tomber dans pending
            AlertHelper.warn("Produit non chargé",
                    "Le produit est encore en cours de chargement.\nPatientez une seconde et réessayez.");
            return;
        }
    ...
    private void chargerProduits(String q) {
        ...
        Task<PageDto<ProduitDto>> task = new Task<>() {
            @Override protected PageDto<ProduitDto> call() throws Exception {
                ...
                return ApiClient.getInstance().get("/produits?q=" + qEnc + "&size=100",
    ...
    private void peuplerFormulaire(ProduitDetailDto d) {
        ...
        // Re-sélectionner le variant modifié s'il y en a un en attente, sinon le premier
        if (!variants.isEmpty()) {
            if (variantIdToReselect != null) {
                final Long toSelect = variantIdToReselect;
                variantIdToReselect = null;
                variants.stream()
                        .filter(v -> toSelect.equals(v.id))
                        .findFirst()
                        .ifPresentOrElse(
                                v -> tableVariants.getSelectionModel().select(v),
                                () -> tableVariants.getSelectionModel().selectFirst());
            } else {
                tableVariants.getSelectionModel().selectFirst();
            }
        }
