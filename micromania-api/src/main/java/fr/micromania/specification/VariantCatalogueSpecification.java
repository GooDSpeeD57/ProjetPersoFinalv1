...
public class VariantCatalogueSpecification {
    ...
    public static Specification<ProduitVariant> avecRecherche(String q) {
        ...
        return (root, query, cb) -> cb.or(
                ...
                cb.like(cb.lower(root.get("ean")),                          pattern)
        );
    }

    /** Recherche exacte par EAN (code-barres). */
    public static Specification<ProduitVariant> avecEan(String ean) {
        if (ean == null || ean.isBlank()) return AUCUN_FILTRE;
        return (root, query, cb) -> cb.equal(root.get("ean"), ean);
    }
