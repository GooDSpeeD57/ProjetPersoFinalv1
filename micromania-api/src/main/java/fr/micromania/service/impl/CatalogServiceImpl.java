...
public class CatalogServiceImpl implements CatalogService {
    ...
    public Page<ProduitSummary> search(String query, Long idCategorie, String niveauAccesMin,
        ...
        if ("nouveautes".equals(tri) || "sortie_desc".equals(tri) || "sortie_asc".equals(tri)) {
            ...
            Sort jpaSort = switch (tri) {
                ...
                default            -> Sort.by("dateSortie").descending();
            };
    ...
    // ── Recherche par EAN (code-barres) ───────────────────────────────────────

    @Override
    public ProduitResponse getProduitByEan(String ean) {
        ProduitVariant variant = variantRepository.findByEanAndActifTrue(ean)
                .orElseThrow(() -> new EntityNotFoundException("Aucun produit pour l'EAN : " + ean));
        return enrichirProduitDetail(variant.getProduit());
    }
    ...
    public ProduitVariantResponse modifierVariant(Long idVariant, CreateVariantRequest request) {
        ...
        // ean peut être explicitement mis à null (pour effacer) — on le met toujours à jour
        variant.setEan(request.ean());
    ...
    @Override
    public List<ProduitSortieItem> getCalendrier() {
        java.time.LocalDate aujourd_hui = java.time.LocalDate.now();

        // On charge tous les variants actifs dont le produit a une dateSortie >= aujourd'hui
        // OU dont le produit est en précommande
        Specification<ProduitVariant> spec = VariantCatalogueSpecification.actif()
                .and((root, cq, cb) -> cb.or(
                        cb.greaterThanOrEqualTo(root.get("produit").get("dateSortie"), aujourd_hui),
                        cb.isTrue(root.get("produit").get("estPreCommande"))
                ));

        List<ProduitVariant> variants = variantRepository.findAll(spec);

        // Tri par dateSortie ASC (nulls last), puis par nom
        variants.sort(Comparator
                .comparing((ProduitVariant v) ->
                                v.getProduit() != null ? v.getProduit().getDateSortie() : null,
                        Comparator.nullsLast(Comparator.naturalOrder()))
                .thenComparing(v -> v.getProduit() != null ? v.getProduit().getNom() : ""));
        ...
        return variants.stream().map(v -> {
            ...
            BigDecimal prix = v.getPrix() == null ? null : v.getPrix().stream()
                    .filter(ProduitPrix::isActif)
                    .map(px -> "OCCASION".equalsIgnoreCase(
                            v.getStatutProduit() != null ? v.getStatutProduit().getCode() : "")
                            ? px.getPrixOccasion() : px.getPrixNeuf())
                    .filter(val -> val != null)
                    .findFirst().orElse(null);

            return new ProduitSortieItem(
                    v.getId(),
                    p.getId(),
                    p.getNom(),
                    p.getSlug(),
