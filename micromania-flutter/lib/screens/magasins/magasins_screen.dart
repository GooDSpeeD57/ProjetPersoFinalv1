...
class _MagasinsScreenState extends State<MagasinsScreen> {
  ...
  Future<void> _charger() async {
    ...
    try {
      ...
      // Charger les horaires en parallèle
      await Future.wait(list.map((m) async {
        m.horaires = await _service.getHoraires(m.id);
      }));
      setState(() {
        _magasins = list;
        _loading = false;
...
class _MagasinCard extends StatefulWidget {
  ...
  @override
  State<_MagasinCard> createState() => _MagasinCardState();
}

class _MagasinCardState extends State<_MagasinCard> {
  bool _horairesDeplies = false;

  @override
  Widget build(BuildContext context) {
    final magasin = widget.magasin;
    final horaireAujourdhui = magasin.horaireAujourdhui;
        ...
        child: Column(
          ...
          children: [
            ...
            if (horaireAujourdhui != null) ...[
              const SizedBox(height: 6),
              GestureDetector(
                onTap: magasin.horaires.isNotEmpty
                    ? () => setState(() => _horairesDeplies = !_horairesDeplies)
                    : null,
                child: Row(
                  children: [
                    Icon(Icons.schedule,
                        size: 14,
                        color: horaireAujourdhui.ferme ? Colors.red[400] : Colors.green[600]),
                    const SizedBox(width: 4),
                    Text(
                      "Aujourd'hui : ${horaireAujourdhui.affichage}",
                      style: TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w600,
                        color: horaireAujourdhui.ferme ? Colors.red[400] : Colors.green[700],
                      ),
                    ),
                    if (magasin.horaires.isNotEmpty) ...[
                      const SizedBox(width: 4),
                      Icon(
                        _horairesDeplies ? Icons.expand_less : Icons.expand_more,
                        size: 16,
            ...
            // ── Planning semaine (déplié) ──
            if (_horairesDeplies && magasin.horaires.isNotEmpty) ...[
              const SizedBox(height: 6),
              Container(
                ...
                child: Column(
                  children: magasin.horaires.map((h) {
                    final estAujourdhui = h.jourSemaine == DateTime.now().weekday;
                    return Padding(
                      ...
                      child: Row(
                        children: [
                          SizedBox(
                            width: 36,
                            child: Text(
                              h.libelleJour.length >= 3 ? h.libelleJour.substring(0, 3) : h.libelleJour,
                              style: TextStyle(
                                fontSize: 12,
            ...
            Row(
              children: [
                ...
                if (widget.onAppeler != null) ...[
                  const SizedBox(width: 8),
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: widget.onAppeler,
